...
class UserscriptAPI {
   ...
   * @param {number} [options.elementThrottleWait=100] `wait` 元素 API 默认 `options.throttleWait`
  ...
  constructor(options) {
    ...
    this.wait = {
      ...
      executeAfterConditionPassed(options) {
        ...
        const task = async () => {
          ...
          if (!terminate) {
            tid = setTimeout(task, options.interval)
          }
        }
        setTimeout(() => {
          tid = setTimeout(task, options.interval)
       ...
       * @param {number} [options.throttleWait=UserscriptAPI.options.elementThrottleWait] 检测节流时间（单位：ms）；节流控制仅当 `repeat` 为 `false` 时生效，设置为 `0` 时禁用节流控制
      ...
      executeAfterElementLoaded(options) {
        options = {
          ...
          multiple: false,
          repeat: false,
          throttleWait: api.options.elementThrottleWait,
    ...
    this.tool = {
       ...
       * 生成节流函数
       * @param {Function} fn 目标函数
       * @param {number} [wait=0] 节流延迟（单位：ms）
