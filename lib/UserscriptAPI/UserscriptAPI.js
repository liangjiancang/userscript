...
class UserscriptAPI {
  ...
  constructor(options) {
    ...
    this.dom = {
       ...
       * 处理 HTML 元素的渐显和渐隐
       * 
       * 仅建议在需要同时修改 `opacity` 和 `display` 的情况下使用。
       * @param {boolean} inOut 渐显/渐隐
       * @param {HTMLElement} target HTML 元素
       * @param {() => void} [callback] 处理完成的回调函数
       * @param {string} [display='unset'] 元素在可视状态下的 `display` 样式
       */
      fade(inOut, target, callback, display = 'unset') {
        ...
        if (inOut) { // 渐显
          ...
          setTimeout(() => {
            ...
            callback?.(success)
          }, displayChanged ? 10 : 0) // 此处的 10ms 是为了保证修改 display 后在浏览器上真正生效，按 HTML5 定义，浏览器需保证 display 在修改 4ms 后保证生效，但实际上大部分浏览器貌似做不到，等个 10ms 再修改 opacity
        } else { // 渐隐
    ...
    this.wait = {
       ...
       * @param {() => *} options.condition 条件，当 `condition()` 返回的 `result` 为真值时达成条件
    ...
    api.wait.waitQuerySelector('head').then(head => {
      const css = head.appendChild(document.createElement('style'))
