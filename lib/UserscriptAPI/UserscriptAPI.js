...
class UserscriptAPI {
  ...
  constructor(options) {
    ...
    this.dom = {
       ...
       * @returns {Promise<HTMLStyleElement>} `<style>`
       */
      async addStyle(css, doc = document, id) {
        const head = await api.wait.waitQuerySelector('head', doc)
    ...
    `, document, `_api_${api.options.id}-style`)
  }
}
