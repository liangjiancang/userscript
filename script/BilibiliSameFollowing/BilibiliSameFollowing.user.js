...
(function() {
	...
	if (location.host == "space.bilibili.com") {
		let ObjectCookie = objectCookies(document.cookie);
				...
				if (resp.code === 0) {
					...
					if (sameFollowings.length > 0 || gm.config.dispMessage) {
						if (sameFollowings.length > 0) {
							...
							if (gm.config.dispInText) {
								dispEl.innerHTML =
									`<div class="gm-pre">共同关注</div><div class="same-following">${sameFollowings.join('，&nbsp;')}</div>`
							} else {
								...
								for (const item of sameFollowings) {
									...
									innerHTML +=
										`<a href="${gm.url.page_space(item.mid)}" target="_blank" class="${className}">${item.uname}</a><span>，&nbsp;</span>`
								}
								dispEl.innerHTML = innerHTML.slice(0, -'<span>，&nbsp;</span>'.length) +
									'</div>'
							}
						} else if (gm.config.dispMessage) {
							dispEl.innerHTML =
								'<div class="gm-pre">共同关注</div><div class="same-following">[ 无 ]</div>'
						}
					}
				} else {
