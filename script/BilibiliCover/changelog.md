...
2. 脚本设置：重新设计脚本选项的分类逻辑。现在封面预览功能可以针对直播间及非直播间分别设置了。
3. 封面预览：当封面尺寸小于预览尺寸时，放大显示。
4. 番剧：优化封面获取流程，修复某些情况下无法响应内容变化的 BUG（或许是B站更新造成）。
5. 直播间：优化针对 `<iframe>` 直播间的处理，排除页面加载不稳定造成的影响。[#95544](https://greasyfork.org/zh-CN/scripts/395575/discussions/95544)
6. 直播间：重新改回懒加载方式，彻底排除页面加载不稳定造成的影响。
7. 代码：提取封面交互代理逻辑。
