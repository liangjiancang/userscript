...
2. 功能实现：在预览图上禁用右键菜单。有了快速复制功能后，右键菜单作用不大，但也不排除对此有需求的用户（如使用浏览器或第三方在右键菜单针对图片提供的功能）。
3. 工作模式：实时预览模式下，在不禁用右键菜单时，右键点击时会加载原图，以便浏览器打开一个针对原图的右键菜单（而非像之前那样打开一个针对包含原图链接的右键菜单）。
4. 工作模式：优化实时预览模式缩略图获取失败时的处理。
5. 封面预览：优化渐入渐隐的处理，从理论上杜绝闪烁的发生（但由于渐入渐隐时间较短，某些情况下还是可能会出现闪烁的「错觉」）。
6. 封面预览：优化最大显示尺寸，并限制最大放大倍数为 `1.5`。
7. 脚本：修复将鼠标移动至按钮/实时预览上后快速点击鼠标左键/中键时，相关操作有概率不会被响应的问题。
8. 脚本：优化封面获取错误时的处理与显示。
9. 脚本：修复脚本提示在懒加载完成时才加载的问题。
10. UI：修复显示被 V3 播放器遮挡的问题。
11. UI：优化文本。
12. 代码：引入类字段声明。
13. API：升级至 V1.8，大幅优化元素等待逻辑，引入网络请求检查、解析、报告功能。详见 [UserscriptAPI 更新日志](https://gitee.com/liangjiancang/userscript/blob/master/lib/UserscriptAPI/changelog.md)。
...
1. 工作模式：修复实时预览模式下，右键菜单中获取到的图片为预览图的问题。
