...
5. 弹出菜单：搜索条件会被保留，下次打开时自动应用，直至页面刷新为止。
6. 脚本：列表页面上方的视频数统计，现在会在相关操作结束后更新。
7. UI：元素内容及布局优化。
...
1. 功能实现：使用时间戳优化移除记录。<br>在历史数据记录中添加时间戳，用于改善移除记录中的数据排序，使得排序以「视频『最后一次』被观察到处于稍后再看的时间点」为基准，而非以「视频『第一次』被观察到处于稍后再看的时间点」为基准；同时也利于数据展示与查看。
