...
2. 功能实现：将视频移动至指定收藏夹。移动后视频会从稍后再看移除。相应地，在弹出菜单及列表页面中增加控制按钮。
3. 排序：重做，改用 `flex` 布局结合 `order` 和 `flex-direction` 实现。
4. 排序：增加「固定」排序类型。排序主逻辑与「默认」相同，但将固定项显示在列表最前方。
5. 列表页面：优化处理流程。
6. 列表页面：显示详细播放进度。
7. 弹出菜单：简单列表支持「时长」「时长↓」排序。
8. 脚本：修复B站某些 API 只接受 `application/x-www-form-urlencoded` 类型编码数据而造成的一些问题。
9. 代码：引入 `aid` 与 `bvid` 互转算法。
10. UI：优化素材，包括且不限于使用矢量图重绘图形。
11. UI：修复显示被 V3 播放器遮挡的问题。
12. API：升级至 V1.5。详见 [UserscriptAPI 更新日志](https://gitee.com/liangjiancang/userscript/blob/master/lib/UserscriptAPI/changelog.md)。
...
5. 弹出菜单：分 P 视频一般显示分 P 总数，鼠标悬浮至上方时显示总时长。
