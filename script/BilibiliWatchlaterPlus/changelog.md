...
6. 移除记录：零碎的问题修复及改进。
7. 批量添加：切换时间单位会动态修改时间值。
8. 批量添加：修复时间过滤步骤中，最大时间范围限制没有考虑时间单位的问题。
9. 批量添加：批量添加管理器参数不再包含时间过滤步骤中的配置值。
10. 重定向：开启后，列表页面中的稍后再看页链接会被替换为常规播放页链接。
11. 重定向：不再停止原页面加载，因为这可能会引起已加载脚本执行错误，也许会造成意外的不良影响。
12. 固定：右键点击固定按钮可以取消所有固定项。
13. 弹出菜单：优化特殊情况下的排版，并增加标题悬浮提示。
14. 脚本：优化初始化流程，避免页面停止加载导致的初始化错误。
15. 脚本：移除脚本菜单「清空稍后再看历史数据」，以后需前往用户设置执行该操作。
16. 代码：使用增强的代码规则。
