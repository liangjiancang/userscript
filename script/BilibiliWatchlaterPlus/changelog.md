...
7. 批量添加：不再加载已存在于稍后再看的视频投稿。
8. 批量添加：若投稿存在于稍后再看历史数据中（仅开启移除记录时），则加载后默认为非选状态。
9. 批量添加：支持使用上下方向键（配合 Alt/Shift/Ctrl）调整时间过滤器。
10. 列表页面：增加设置项「显示批量添加管理器按钮」「显示自动移除控制器」。
11. 收藏：改用对话框组件实现稍后再看收藏夹配置。
12. 用户设置：优化设置项排布及内部处理逻辑。
13. 脚本：修复在 Firefox 上有概率无法正常执行的问题。
14. 脚本：增强内部数据校验。
