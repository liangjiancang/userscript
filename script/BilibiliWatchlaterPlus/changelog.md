...
2. 批量添加：对于加载出来的稿件项目，支持鼠标右键打开稿件、鼠标中键后台打开稿件并取消勾选状态。
3. 填充缺失状态：适配新版顶栏动态面板，适配新版用户空间动态。
4. 外部：`UserScriptAPIBase` 更新至 V1.3。
...
3. 列表导出：完善设置框中的「示例」说明，补充控制导出顺序的示例。
4. 外部：`UserScriptAPIWeb` 更新至 V1.4。
