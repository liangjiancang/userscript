...
8. 弹出菜单：优化菜单弹出流程以降低误触率，避免用户将鼠标从浏览器工具栏移回网页时误触弹出菜单。
9. 弹出菜单：优化菜单关闭的渐隐效果。
10. 重定向：兼容 URL 中 BV 号被第三方修改为 AV 号的情况。
11. 脚本：在列表页面中点击视频右方移除按钮后，视频会移动至列表末尾，并以去色加删除线形式来表示。
12. 脚本：列表页面上方的视频数统计，现在会在相关操作结束后更新。
13. 脚本：优化主流程。
14. 脚本：优化错误处理流程。
15. 脚本：优化初始化流程。
16. 脚本：优化脚本菜单运行流程。
17. 脚本：优化 URL 匹配。
18. 脚本：进一步优化列表页面中已失效视频的交互。
19. 代码：优先使用函数声明而非函数表达式来定义内部函数。
20. UI：元素内容及布局优化。
21. API：升级至 V1.3。详见 [UserscriptAPI 更新日志](https://gitee.com/liangjiancang/userscript/blob/master/lib/UserscriptAPI/changelog.md)。
