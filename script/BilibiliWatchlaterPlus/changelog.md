...
8. 弹出面板：稿件统计常规状态下显示为 `稍后再看稿件`，执行搜索时显示为 `搜索出来的稍后再看稿件/搜索出来的全部非移除稿件`。
9. 列表页面：稿件统计常规状态下显示为 `（稍后再看稿件/全部稿件）`，执行搜索时显示为 `（搜索出来的稍后再看稿件/搜索出来的全部稿件/全部稿件）`。
10. 列表页面：修复点击「刷新列表」按钮加载搜索框默认值时，没有修改搜索框显示状态的问题。
11. 快速切换：适配B站新版各播放页面修改。
12. 代码：`config` / `configMap` 机制不再要求在「用户设置」中有对应配置项。
13. 约定：「选项」->「配置项」。
14. 外部功能：由于B站新版页面顶栏已原生固定，移除 [B站顶栏固顶](https://greasyfork.org/zh-CN/scripts/430292) 入口。
