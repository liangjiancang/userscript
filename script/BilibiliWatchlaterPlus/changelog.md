...
1. 填充缺失状态：适配B站动态主页改版，支持新版顶栏动态面板。
2. 批量添加：适配B站动态主页改版。
3. 列表页面：新增选项「控制栏随页面滚动」，以达成类似于固顶的效果。
4. 弹出面板：修复降序排序时无法上下滚动的问题。
5. 顶栏兼容：不再作为「弹出面板」的子选项，而是作为全局性的第三方顶栏兼容控制项，归类到「相关调整」中。
6. 约定：「弹出菜单」->「弹出面板」。~~额，之前的叫法明显是不对的啊！！！~~
7. 约定：「顶栏XX入口弹出面板」可缩写为「XX面板」。
8. UI：滚动条样式设置现在会影响新版顶栏各入口的弹出面板。
9. UI：优化文本。
