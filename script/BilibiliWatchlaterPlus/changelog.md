...
2. 批量添加：支持加载已订阅的合集视频。
3. 批量添加：增加平均请求间隔。根据 [B站点赞批量取消](https://greasyfork.org/zh-CN/scripts/445754) 脚本编写过程中遭遇的情况，原先使用的默认值及最小值显得有些不够保守。
4. 批量添加：允许「筛选」「过滤」关键词中包含非首末空白符。
5. 批量添加：优化不同标签页中脚本实例对同步时间进行同步的处理。
