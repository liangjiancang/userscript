...
1. 自动移除：顶栏入口弹出菜单中增加一个「自动移除」按钮，用于直接控制在弹出菜单中打开视频时是否执行自动移除逻辑，也就是给予用户一个临时切换的开关。相应地，为该功能新增「在弹出菜单底部显示自动移除控制」选项。
2. 自动移除：列表页面中，现在被自动移除的视频不会从页面上消失，而是改为以去色加删除线形式来表示。
3. 自动移除：优化列表页面中的自动移除处理。
4. 弹出菜单：优化无列表数据及无搜索结果时的显示。
5. 弹出菜单：增加选项「在弹出菜单中显示被移除的视频」。开启后搜索条件会被保留，下次打开时自动应用，直至页面刷新为止。
6. 重定向：兼容 URL 中 BV 号被第三方修改为 AV 号的情况。
7. 脚本：列表页面上方的视频数统计，现在会在相关操作结束后更新。
8. 脚本：优化错误处理流程。
9. UI：元素内容及布局优化。
10. API：升级至 V1.3。详见 [UserscriptAPI 更新日志](https://gitee.com/liangjiancang/userscript/blob/master/lib/UserscriptAPI/changelog.md)。
