...
3. 自动移除：列表页面中，现在被自动移除的视频不会从页面上消失，而是改为以去色加删除线形式来表示。
...
9. 脚本：列表页面上方的视频数统计，现在会在相关操作结束后更新。
10. 脚本：优化主流程。
11. 脚本：优化错误处理流程。
12. 脚本：优化初始化流程。
13. 脚本：优化 URL 匹配。
14. UI：元素内容及布局优化。
15. API：升级至 V1.3。详见 [UserscriptAPI 更新日志](https://gitee.com/liangjiancang/userscript/blob/master/lib/UserscriptAPI/changelog.md)。
