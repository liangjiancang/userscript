...
2. 脚本：优化初始化流程。
3. 功能实现：隐藏分段信息。
4. 功能移除：隐藏「热度」曲线。现在总是隐藏。
5. 代码：优先使用函数声明而非函数表达式来定义内部函数。
...
3. 脚本：大幅简化页面切换处理流程。
4. 脚本：优化 URL 匹配。
5. 脚本：明确不支持分段进度条——主要是因为按目前的实现方式要支持分段进度条非常困难。另一方面，既然 UP 主在某视频启用了分段进度条，本身就说明该视频向观众提供章节并不影响体验，换句话说这种视频理应是没有剧透问题的；否则，UP 主才是「剧透」的元凶。以后可能会采用更合理的实现方式将脚本重写一遍，到时候也许会重新考虑支持分段进度条。
6. UI：优化脚本控制的外观。
7. UI：设置页布局优化。
