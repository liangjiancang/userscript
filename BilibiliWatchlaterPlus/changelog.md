...
1. 自动移除：列表页面中增加一个「打开后自动移除」按钮，用于直接控制在列表页面中打开视频时是否执行自动移除逻辑，也就是给予用户一个临时切换的开关。相应地，为该功能新增「彻底禁用自动移除」选项。
...
>
> 其实也没什么好抒发的，只是希望官方在做改动前能够搞清楚什么才是用户想要的。网页版改成奇奇怪怪的样子，尚且有脚本可以修修补补；这段时间 APP 上的稍后再看被「优化」成了一坨屎的样子，可真把我给恶心到了。
>
> 到此为止！
